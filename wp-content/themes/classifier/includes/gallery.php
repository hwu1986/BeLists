...
$photo_size = 'single-ad';		// The WP "size" to use for the large image
...
if ( !empty($attachments) ) :
	...
	foreach ( $attachments as $att_id => $attachment ) {
		...
		if ( get_option('colabs_resize') == "true" && $use_timthumb == "true" )
  			$photo_output .= '<div><a href="'. $src[0] .'" rel="lightbox-group" class="thickbox" title="'.$attachment->post_excerpt.'">' . colabs_image( 'src=' . $src[0] . '&width='.$width_setting.'&class=single-photo&return=true' ) . '</a>'.$caption.'</div>';
		else
