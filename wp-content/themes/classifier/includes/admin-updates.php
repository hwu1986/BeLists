...
if ( !function_exists('colabs_upgrade_all') ) :
/**
 ...
 * @since 3.1.0
 */
function colabs_upgrade_all() {
	global $colabs_abbr, $colabs_version, $colabs_current_db_version, $colabs_db_version, $wp_rewrite;
	
	$colabs_current_db_version = get_option($colabs_abbr.'_db_version');

	// We're up-to-date. Nothing to do.
	if ( $colabs_db_version == $colabs_current_db_version )
		return;

	// If the version is not set in the db, try to guess the version.
	if ( empty($colabs_current_db_version) )
		$colabs_current_db_version = 0;

	if ( $colabs_current_db_version < 1200 )
	    colabs_upgrade_310();

	if ( $colabs_current_db_version < 1280 )
	    colabs_update_advanced_search_db();

	if ( $colabs_current_db_version < 1290 )
	    colabs_upgrade_317();
...
/**
 * Execute changes made in Classifier 3.1.0.
 *
 * @since 3.1.0
 */
function colabs_upgrade_310() {
	global $wpdb, $colabs_abbr, $colabs_version;

    $wpdb->query( "ALTER TABLE " . $wpdb->prefix . "colabs_ad_meta ADD `field_search` int(10) NOT NULL" );
    $wpdb->query( "ALTER TABLE " . $wpdb->prefix . "colabs_ad_fields ADD `field_min_length` int(11) NOT NULL" );
    $wpdb->query( "ALTER TABLE " . $wpdb->prefix . "colabs_ad_fields ADD `field_validation` LONGTEXT NULL" );
    $wpdb->query( "ALTER TABLE " . $wpdb->prefix . "colabs_ad_packs ADD `pack_type` VARCHAR(255) NOT NULL" );
    $wpdb->query( "ALTER TABLE " . $wpdb->prefix . "colabs_ad_packs ADD `pack_membership_price` DECIMAL(10,2) UNSIGNED NOT NULL DEFAULT '0'" );
    ...
    if ( get_option($colabs_abbr.'_membership_purchase_url') == false ) update_option($colabs_abbr.'_membership_purchase_url', 'membership');
    if ( get_option($colabs_abbr.'_membership_purchase_confirm_url') == false ) update_option($colabs_abbr.'_membership_purchase_confirm_url', 'membership-confirm');
    ...
    */
    ...
    if ( !empty( $cur_ex_pages ) ) {
        ...
        $ex_pages = get_option( $colabs_abbr.'_excluded_pages' );
        ...
        update_option( $colabs_abbr.'_excluded_pages', colabsthemes_clean( $all_ex_pages ) );
    }
    
    update_option( 'colabs_db_version', 1200 );
    update_option( $colabs_abbr.'_version', $colabs_version );
...
/**
 * These are functions to run for Classifier 3.0.5.
 *
 * @since 3.0.5
 */
...
function colabs_upgrade_305_nag() {
	...
	if ( !current_user_can('manage_options') ) 
        ...
        $wpdb->get_results( "SELECT ID FROM $wpdb->posts WHERE post_type = '".COLABS_POST_TYPE."' LIMIT 1" );
		...
		$incats = colabs_get_blog_cat_ids();
		...
		<div id="message2" class="updated">
			...
			<h3>Classifier upgrade required</h3>
			<p>Your database needs to be updated before using this version of Classifier. It's important to first back-up your database <strong>BEFORE</strong> running this upgrade tool. We are not responsible for any lost or corrupt data. We recommend using the <a href='http://wordpress.org/extend/plugins/wp-db-backup/' target='_blank'>WP-DB plugin</a> to easily back-up your database. To install it directly from within WordPress, just go to your '<a href='plugin-install.php'>Add New</a>' plugins page and search for 'WP-DB-Backup'. For more instructions, see <a href='http://codex.wordpress.org/Backing_Up_Your_Database#Installation' target='_blank'>this page</a>.</p>

			<h3>What will this upgrade do?</h3>
			<p>Classifier takes advantage of the new custom post types and taxonomies available in WordPress so we need to migrate your ads, ad categories, and copy your tags from 'posts' to 'ads'. See the new 'Ads' menu group in your left-hand sidebar? Yep, that's where we're going to move them to.</p>
			...
			<p><small><?php _e('Note: This message will not disappear until you have upgraded your database.', 'colabsthemes'); ?></small></p>

		</div>

	<script type="text/javascript">
        /* <![CDATA[ */
            function confirmUpdate() { return confirm("Are you sure you wish to run the Classifier upgrade script? Promise you already backed up your database? It's better to be safe than sorry! :-)"); }
        /* ]]> */
    </script>
<?php

}
add_action('admin_notices', 'colabs_upgrade_305_nag', 3);


// convert post types to ad_listing
function colabs_convert_posts2Ads() {
	global $wpdb, $colabs_version;
	...
	$blogCatIDs = array();
	$blogCatIDs = colabs_get_blog_cat_ids_array();
	...
	foreach ( $cat_ids as $cat_id ) {
		...
		if ( !in_array($cat_id, $blogCatIDs) ){
			$wpdb->update( $wpdb->term_taxonomy, array( 'taxonomy' => COLABS_TAX_CAT ), array( 'term_id' => $cat_id ) );
	...
	foreach ( $theposts as $post ) {
		...
		if (!in_array($cat_id, $blogCatIDs)){
			...
			if (!empty($newTags))
				wp_set_post_terms($post->ID, $newTags, COLABS_TAX_TAG);

			//now change the post to an ad
			set_post_type($post->ID, COLABS_POST_TYPE);
	...
	// update_option('colabs_version_old', $colabs_version);
?>
...
/**
* Classifier 3.1.0 geocoding migration script  
*
* @since 3.1.0
*/
function colabs_update_advanced_search_db() {
	global $wpdb, $colabs_current_db_version;
	
	// get the Classifier db version number
	$colabs_current_db_version = get_option( 'colabs_db_version' );
	
	colabs_create_geocode_table();
	
	$post_ids = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_type = '" . COLABS_POST_TYPE . "' ORDER BY ID ASC" ) );
	if ( $post_ids ) {
	?>
        <div class="wrap">
        <?php screen_icon( 'themes' ); ?>
        <h2><?php _e( 'Classifier Update', 'colabsthemes' ); ?></h2>
        <p class="info"><?php _e( 'Geocoding ad listing addresses to enable the advanced search radius feature to work. This process queries Google Maps to get longitude and latitude coordinates based on each ad listings address. Please be patient as this may take a few minutes to complete.', 'colabsthemes' ); ?></p>
        <?php
    
        foreach ( $post_ids as $post_id ) {
            if ( ! colabs_get_geocode( $post_id ) ) {
                ...
                if ( $address ) {
                    printf( '<p>' . __( "Ad #%d - %s ", 'colabsthemes' ), $post_id, $address );
                    ...
                    if ( 'OK' == $geocode->status ) {
                        ...
                        $category = get_the_terms( $post_id, COLABS_TAX_CAT );
                        ...
                        echo ' &raquo; <font color="green">' . __( 'Geocoding complete.', 'colabsthemes' ) . '</font>';
                    } else {
                        echo ' &raquo; <font color="red">' . __( 'Geocoding failed - address not found.', 'colabsthemes' ) . '</font>';
                    }
                    echo '</p>';
                }
            }
        ...
        echo '<br /><strong>' . __( 'Database update complete.', 'colabsthemes' ) . '</strong><br />';
        echo '<small>' . __( 'Please note: Ads that failed during this process will not show up during a radius search since the address was invalid. Ignore this message for new installs.', 'colabsthemes' ) . '</small>';
        echo '<br /><br /><a class="button" href="admin.php?page=colabsthemes">' . __( 'Continue to Your Dashboard', 'colabsthemes' ) . '</a>';
        ?>
        </div>
	<?php
	    update_option( 'colabs_db_version', 1280 );
...
function colabs_create_geocode_table() {
	global $wpdb;
...
/**
 * Execute changes made in Classifier 3.1.7.
 *
 * @since 3.1.7
 */
function colabs_upgrade_317() {
	global $wpdb, $colabs_abbr, $colabs_version;

	$sql = $wpdb->prepare( "SELECT field_name FROM ". $wpdb->prefix . "colabs_ad_fields WHERE field_type = 'checkbox' " );
  ...
  if ($results) :
	...
	update_option( 'colabs_db_version', 1290 );
