...
function colabs_process_paypal_transaction() {
    global $wpdb;
    ...
    if ( $_POST['txn_id'] ) {
		...
		$results = $wpdb->get_var( $wpdb->prepare( "SELECT txn_id FROM {$wpdb->prefix}colabs_order_info WHERE txn_id = %s LIMIT 1", colabsthemes_clean( $_POST['txn_id'] ) ) );

        if ( !$results ) :

	    // @todo Change to Insert
            $sql = $wpdb->prepare( "INSERT INTO " . $wpdb->prefix . "colabs_order_info" .
                    ...
                    colabsthemes_clean($aid),
                    colabsthemes_clean($_POST['first_name']),
                    colabsthemes_clean($_POST['last_name']),
                    colabsthemes_clean($_POST['payer_email']),
                    colabsthemes_clean($_POST['residence_country']),
                    colabsthemes_clean($_POST['transaction_subject']),
                    colabsthemes_clean($_POST['item_name']),
                    colabsthemes_clean($_POST['item_number']),
                    colabsthemes_clean($_POST['payment_type']),
                    colabsthemes_clean($_POST['payer_status']),
                    colabsthemes_clean($_POST['payer_id']),
                    colabsthemes_clean($_POST['receiver_id']),
                    colabsthemes_clean($_POST['parent_txn_id']),
                    colabsthemes_clean($_POST['txn_id']),
                    colabsthemes_clean($_POST['mc_gross']),
                    colabsthemes_clean($_POST['mc_fee']),
                    colabsthemes_clean($_POST['payment_status']),
                    colabsthemes_clean($_POST['pending_reason']),
                    colabsthemes_clean($_POST['txn_type']),
                    colabsthemes_clean($_POST['tax']),
                    colabsthemes_clean($_POST['mc_currency']),
                    colabsthemes_clean($reason_code),
                    colabsthemes_clean($_POST['custom']),
                    colabsthemes_clean($_POST['test_ipn']),
                    $payment_date,
                    current_time('mysql')
                    );
        ...
        // @todo send through prepare
            ...
            $update = "UPDATE " . $wpdb->prefix . "colabs_order_info SET" .
                    " payment_status = '" . $wpdb->escape(colabsthemes_clean($_POST['payment_status'])) . "'," .
                    " mc_gross = '" . $wpdb->escape(colabsthemes_clean($_POST['mc_gross'])) . "'," .
                    " txn_type = '" . $wpdb->escape(colabsthemes_clean($_POST['txn_type'])) . "'," .
                    " reason_code = '" . $wpdb->escape(colabsthemes_clean($reason_code)) . "'," .
                    " mc_currency = '" . $wpdb->escape(colabsthemes_clean($_POST['mc_currency'])) . "'," .
                    " test_ipn = '" . $wpdb->escape(colabsthemes_clean($_POST['test_ipn'])) . "'," .
...
add_action( 'colabs_process_transaction_entry', 'colabs_process_paypal_transaction' );

do_action('colabs_process_transaction_entry', $_REQUEST);


// prepare array with transaction data
function colabs_prepare_transaction_entry($order_vals) {
  global $wpdb;
  ...
  if ( is_array($order_vals) ):
    ...
    $trdata['payment_type'] = $order_vals['colabs_payment_method'];
    ...
    $trdata['mc_currency'] = get_option('colabs_curr_pay_type');
...
function colabs_add_transaction_entry($trdata) {
  global $wpdb;
  ...
  if ( isset($trdata['txn_id']) ) {
    ...
    $order_table = $wpdb->prefix . 'colabs_order_info';
    ...
    if ( !$results ) :
      ...
      $allowed_fields = array( 'txn_type', 'payment_status', 'mc_gross', 'mc_currency', 'reason_code', 'test_ipn', 'payment_date' );
      $data = array();
